...
Two write endpoints. `POST /admin/notifications/:id/retry`: atomic conditional UPDATE keyed on `status='FAILED'` — sets status=PENDING, attempts=0, clears processing*started_at + last_error, appends a `MANUAL_RETRY` history entry, returns the new row. If the conditional UPDATE matches no rows, a follow-up SELECT decides 404 (row missing) vs 409 (status≠FAILED). `POST /admin/dlq/inbox/republish`: drains `notifier.user-created.dlq` (or selectively by `userId` from optional body `{ ids: [...] }`) and republishes matched messages via a notifier-side `UserCreatedProducer` to `users.events / user.created`. Unmatched and invalid messages are \_re-published to `notifications.dlx`* (recreating the entry at the tail) rather than nack-requeued — RabbitMQ requeues at the head and would loop the drain on the same message. Selective filter matches the **payload's `userId`**, not the AMQP `messageId` — `messageId` isn't always set (e.g., messages planted via the management UI). The drain takes one pass capped by `checkQueue.messageCount`. Returns `{republished, failed, skipped}`. The `BaseZodValidationInterceptor` change from Phase 11 (folding `request.params` into the validation payload) lets the same `GetNotificationParamSchema` validate the retry route's `:id`.
...
## ~ Phase 13: Optional remainder (partial)

**User stories**: 35, 36

### What to build

A grab-bag of production-hardening improvements that are independent of each other. Each can be picked up individually. Three were shipped in this run; three were explicitly skipped (noted below). **Read replicas**: `postgres-replica` container runs as streaming-replication standby of `postgres`; `*_READ_DB_URL` env templates point at the standby. **Full graceful shutdown**: `RmqConsumer` base implements `OnApplicationShutdown` — cancels the broker consumer, awaits in-flight handlers (10s drain timeout), then closes the channel. PrismaClient instances are factory-created with a monkey-patched `onModuleDestroy` that calls `$disconnect`. `SchedulerService` implements `OnModuleDestroy` and calls `SchedulerRegistry.deleteCronJob` for each registered cron. Shutdown logs go through `process.stdout.write` rather than pino so they land on stdout even if pino's transport worker has torn down. **Alternate-exchange**: producer + consumer decorators gain optional `exchangeArgs`; the two scheduler producers + the two cron consumers all assert `system.cron` with `arguments: { 'alternate-exchange': 'unrouted.alt' }`. `SchedulerTopologyService` declares the `unrouted.alt` fanout exchange + `unrouted` catch-all queue at boot. **Skipped**: K8s manifests (would need a kind/minikube cluster to validate), scheduler leader-election (requires switching scheduler to N>1 + advisory-lock plumbing), live/ready probe split (already wired in Phase 1, would just be k8s YAML).

### Acceptance criteria

- [x] **Read replicas**: standby visible on `localhost:5433`; primary write propagates to standby in <1s; standby refuses INSERT (`ERROR: cannot execute INSERT in a read-only transaction`) _(verified: `phase13-replica` user POST → row appears on both primary + replica psql; replica's `pg_is_in_recovery()=t`, `pg_replication_slots.active=t`)_
- [x] **Full graceful shutdown**: SIGTERM to the Nest process emits 3× `[shutdown] consumer ... got SIGTERM` + 3× `consumer ... drained cleanly` + 2× `prisma client disconnected` + 2× `cron job stopped` for users + notifier + scheduler respectively, in order, before exit _(verified — direct `kill -TERM <pid>` against the dist/main.js process; in dev compose, `docker stop` only signals the npm wrapper which doesn't propagate, so prod runtime stage `node dist/main.js` is the canonical demo path)_
- [x] **Alternate-exchange**: `system.cron` exchange now carries `arguments: {'alternate-exchange': 'unrouted.alt'}`; publishing with routing key `cron.bogus` lands in the `unrouted` queue with the original routing key preserved; valid keys (`cron.users`, `cron.notifier`) still route to their consumer queues so the unrouted queue stays at 0 in steady state
- [ ] **K8s manifests** — skipped (would need kind/minikube)
- [ ] **Scheduler leader-election** — skipped (requires advisory-lock + scheduler N>1)
- [ ] **K8s probes** — skipped (already split since Phase 1; only k8s YAML left)
